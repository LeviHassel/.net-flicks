    ...
    public class AccessorMapper : Profile
    {
        public AccessorMapper()
        {
            ...
            CreateMap<RootObject, Movie>()
                ...
                .ForMember(dest => dest.ReleaseDate, opt => opt.MapFrom(src => Convert.ToDateTime(src.release_date))) //see if this works
                .ForMember(dest => dest.Runtime, opt => opt.MapFrom(src => TimeSpan.FromMinutes(src.runtime))) //see if this works
                .ForMember(dest => dest.ImageUrl, opt => opt.MapFrom(src => "https://image.tmdb.org/t/p/w600_and_h900_bestv2/" + src.poster_path)) //see if this works
                .ForMember(dest => dest.TrailerUrl, opt => opt.MapFrom(src => "https://www.youtube.com/watch?v=" + src.videos.results.First().key)) //see if this works
